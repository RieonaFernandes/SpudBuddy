...
export function generateWorkout(args) {
    const { muscles, poison: workout, objective } = args
    //get the names of all the exercises
    let exer = Object.keys(exercises);
    //get all the exercises where your workout environment is not home.
    exer = exer.filter((key) => exercises[key].meta.environment !== "home");
    let includedTracker = [];
    let numSets = 5;
    let listOfMuscles;

    //Handel array and objects seperately
